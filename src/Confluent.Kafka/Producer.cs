...
namespace Confluent.Kafka
{
    ...
    internal class SerializingProducer<TKey, TValue> : ISerializingProducer<TKey, TValue>
    {
        ...
        // public void ProduceAsync(string topic, TKey key, TValue val, DateTime timestamp, int partition, bool blockIfQueueFull, IDeliveryHandler<TKey, TValue> deliveryHandler)
        //     => Produce(topic, key, val, timestamp, partition, blockIfQueueFull, deliveryHandler);

        // public void ProduceAsync(string topic, TKey key, TValue val, DateTime timestamp, int partition, IDeliveryHandler<TKey, TValue> deliveryHandler)
        //     => Produce(topic, key, val, timestamp, partition, true, deliveryHandler);

        // public void ProduceAsync(string topic, TKey key, TValue val, DateTime timestamp, IDeliveryHandler<TKey, TValue> deliveryHandler)
        //     => Produce(topic, key, val, timestamp, Producer.RD_KAFKA_PARTITION_UA, true, deliveryHandler);
    ...
    public class Producer<TKey, TValue> : ISerializingProducer<TKey, TValue>, IDisposable
    {
        ...
        // public void ProduceAsync(string topic, TKey key, TValue val, DateTime timestamp, int partition, bool blockIfQueueFull, IDeliveryHandler<TKey, TValue> deliveryHandler)
        //     => serializingProducer.ProduceAsync(topic, key, val, timestamp, partition, blockIfQueueFull, deliveryHandler);

        // public void ProduceAsync(string topic, TKey key, TValue val, DateTime timestamp, int partition, IDeliveryHandler<TKey, TValue> deliveryHandler)
        //     => serializingProducer.ProduceAsync(topic, key, val, timestamp, partition, deliveryHandler);

        // public void ProduceAsync(string topic, TKey key, TValue val, DateTime timestamp, IDeliveryHandler<TKey, TValue> deliveryHandler)
        //     => serializingProducer.ProduceAsync(topic, key, val, timestamp, deliveryHandler);
